...
");
            ...
            $result = $conn->query("select c.id_motorista,c.titulo,c.id as id_carona,a.id as id_aplicacao,a.mensagem,a.id_passageiro,u.id as id_usuario,u.nome,a.status,c.vagas from aplicacao a
inner join carona c on c.id= a.id_carona
inner join usuario u on u.id=a.id_passageiro
WHERE c.id_motorista = $id_motorista AND c.id = $id_carona");
 $aplicacoes = [];
            while ($row = $result->fetch_assoc()) {
                $aplicacoes[] = $row;
            }
